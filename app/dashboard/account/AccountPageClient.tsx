...
export default function AccountPage() {
  ...
  const planDisplay = plan === "pro" ? "프로" : plan === "grace" ? "결제 유예" : "무료";
  ...
  async function sendPasswordResetEmail() {
    ...
    try {
      ...
      pushToast("비밀번호 재설정 메일을 보냈어요.");
    } catch (error) {
      const message = error instanceof Error ? error.message : "비밀번호 재설정 메일 전송에 실패했어요.";
      pushToast(message);
    } finally {
  ...
  async function openBillingPortal() {
    ...
    try {
      ...
      const body = text ? (JSON.parse(text) as { ok?: boolean; url?: string; message?: string }) : null;
      if (!res.ok || body?.ok !== true || !body?.url) {
        throw new Error(body?.message ?? "결제 관리 페이지를 열 수 없어요.");
      }
      window.location.href = body.url;
    } catch (error) {
      const message = error instanceof Error ? error.message : "결제 관리 페이지 이동에 실패했어요.";
      pushToast(message);
    } finally {
        ...
        {loading ? (
          ...
          <div className="mt-6 space-y-3">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
              ...
              <div className="mt-3 text-xs text-zinc-500">사용자 ID: {userId}</div>
              {createdAt ? <div className="mt-1 text-xs text-zinc-500">가입일: {new Date(createdAt).toLocaleString()}</div> : null}
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
              <div className="text-sm font-medium text-zinc-100">보안</div>
              <div className="mt-2 text-sm text-zinc-400">비밀번호 변경 및 계정 보안 작업을 할 수 있어요.</div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
              ...
              <div className="mt-3 space-y-3">
                ...
                <div>
                  <div className="text-sm text-zinc-300 mb-1">시간대</div>
                  <input
                    value={timezone}
